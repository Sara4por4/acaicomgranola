@php
    /*
        ...
        // the collection containing the records
        'records' => $records,
        ...
        // the CRUD actions available from the records list
        'actions' => [
            'sort' => 'controller@method',
            'activate' => 'controller@method',
                    ...
                    >
                        @foreach($records as $record)
                            <tr data-id="{{ $record->id }}" id="item-{{ $record->id }}" class="ui-state-default">
                                ...
                                @if(!empty($toggleable))
                                    <td>
                                        {!! Form::model($record->id, ['method' => "PATCH", 'action' => ['\App\Packages\ToggleManager\ToggleManagerController@toggle', $record->id], 'class'=>'activate-form']) !!}
                                ...
                                @if(!empty($actions['edit']) || !empty($actions['destroy']))
                                    <td class="text-right">
                                        ...
                                        @if(!empty($actions['edit']))
                                            ...
                                            @if(Request::segment(2)=='candidates')
                                                <a href="{{ action($actions['edit'], ['id' => $record->id]) }}" class="btn btn-info btn-xs"><i class="fa fa-eye"></i> Ver Candidatura </a>
                                            @else
                                                <a href="{{ action($actions['edit'], ['id' => $record->id]) }}" class="btn btn-info btn-xs"><i class="fa fa-pencil"></i> Editar </a> 
                                            @endif
                                        @endif
                                        <!-- End: Edit action -->
